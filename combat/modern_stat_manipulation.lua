...
return function(mod)
  ...
  -- the mon itself (HP, and Power Shift's/Syrup Bomb's stored state).
  ...
  -- Power Shift restore on switch-out / battle end (its fields are on the
  -- mon, so the switch path's own clearVolatile cannot reach them).
  mod.events:on("battle.battler_switched", function(ev)
    local battle = ev and ev.battle
    if not battle then return end
    revertPowerShift(battle, ev.previous, isGen2Battle(battle))
  end)
  mod.events:on("battle.ended", function(ev)
    ...
    local actives = (mod.exports.allActiveBattlers and mod.exports.allActiveBattlers(battle))
      or { battle.player, battle.enemy }
    for i = 1, #actives do revertPowerShift(battle, actives[i], gen2) end
    local parties = { battle.party, battle.enemyParty, battle.playerParty }
    for p = 1, #parties do
      local party = parties[p]
      if type(party) == "table" then
        for i = 1, #party do revertPowerShift(battle, party[i], gen2) end
      end
    end
  end)
