  ...
  mod.events:on("battle.damage_dealt", function(ev)
    ...
    local ok, err = pcall(function()
      ...
      if facts.volatileStatus == "flinch" then
        ...
        -- battler's own field. A bare `battle.volatile and ...` test is NOT
        -- equivalent -- Gen 1's real BattleState has no such method, but a
        -- test mock may, which would strand the flinch where nothing reads it.
        if gen2 then
